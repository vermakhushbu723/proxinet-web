...
export function saveFile(file) {
  return new Promise((resolve, reject) => {
    ...
    const upload = bucket().openUploadStream(filename, { metadata: { contentType: file.mimetype } });
...
export async function streamFile(fileId, res) {
  ...
  const [meta] = await bucket().find({ _id: id }).toArray();
  ...
  await new Promise((resolve, reject) => {
    bucket().openDownloadStream(id).on('error', reject).on('end', resolve).pipe(res);
  });
}

export async function deleteFile(fileId) {
  if (!fileId) return;
  try { await bucket().delete(new mongoose.Types.ObjectId(String(fileId))); } catch { /* already gone */ }
}
